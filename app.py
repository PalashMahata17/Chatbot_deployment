...
def load_model_and_tokenizer():
    ...
    if torch.cuda.is_available():
        ...
        st.warning("No CUDA GPU detected. Loading model on CPU. This will be slower and consume more RAM.")
        # Load model without quantization if no GPU, directly to CPU
        model = AutoModelForCausalLM.from_pretrained(
            model_name,
            device_map=None, # Explicitly no device map
            torch_dtype=torch.float32, # Use float32 for CPU for stability/compatibility
            trust_remote_code=True,
        )
